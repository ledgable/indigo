...
class ChainReader(BaseClass):
	...
	# add transaction into space... for future use
	
	def addTransactionsToHistory(self, transactions):
		pass
	...
	def reload(self):
		...
		while (True):
			...
			if (len(transactions_) == 0):
				...
				self.addTransactionsToHistory(transactions_)
	...
	def writeTransactionsToChain(self, transactionsToWrite=None):
		...
		for transaction_ in transactionsToWrite:
			...
			if (oktowrite_):
				...
				self.addTransactionsToHistory([transaction_])
