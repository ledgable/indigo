...
class DataNodeController(NodeController):
	...
	@endpoint(1, False, True, None, "get", "^/api/(?P<chainid>[0-9a-f][^-&*/\%]*)/config", "Get chain configuration info")
	...
	@endpoint(1, False, True, None, "get", "^/api/(?P<chainid>[0-9a-f][^-&*/\%]*)/hashes", "Get chain hashes")
	...
	@endpoint(1, False, True, None, "get", "^/api/(?P<chainid>[0-9a-f][^-&*/\%]*)/transactions/key/(?P<classid>[0-9a-f][^-&*/\%]*)/(?P<key>[0-9a-z_][^-&*/\%]*)/(?P<value>[0-9a-z_-][^&*/\%]*)", "Get transactions for block for key with value")
	def getTransactionsForKeyAndValue(self, postData=None, appVars=None, chainid=None, classid=None, key=None, value=None):
			
		transactions_ = self.transactionsWithKeyValue(chainid, classid, key, value, "==")
